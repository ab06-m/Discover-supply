  ...
  return (
    <div style={{ padding: '28px 32px', maxWidth: 1100 }}>
      <PageHeader title="Inventory" subtitle={`${PRODUCTS.length} products`}
        actions={<><Btn variant="outline" size="sm"><Icons.Upload /> Import</Btn><Btn size="sm"><Icons.Plus /> Add product</Btn></>}
      />

      {(lowCount > 0 || outCount > 0) && (
        <div style={{ marginTop: 16, padding: '10px 16px', background: C.warningBg, border: `1px solid ${C.warning}`, borderRadius: 10, display: 'flex', alignItems: 'center', gap: 10, fontSize: 14, color: C.warningText }}>
          ...
          <button onClick={() => setTab('low')} style={{ marginLeft: 'auto', fontFamily: 'inherit', fontSize: 13, fontWeight: 500, color: C.warningText, background: 'none', border: 'none', cursor: 'pointer', textDecoration: 'underline' }}>View alerts</button>
        </div>
      )}
      ...
      <div style={{ display: 'flex', gap: 8, margin: '14px 0', flexWrap: 'wrap' }}>
        <Input placeholder="Search by name or SKU…" value={q} onChange={e => setQ(e.target.value)} icon={<Icons.Search />} style={{ width: 260 }} />
      ...
      <Card style={{ overflow: 'hidden' }}>
        {filtered.length === 0 ? (
          <EmptyState icon={Icons.Package} title="No products match" description="Adjust your filters or add a new product." />
        ) : (
                  ...
                  <tr key={p.id}
                    ...
                    <td style={{ padding: '11px 16px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        ...
                        <span style={{ fontSize: 14, fontWeight: 500, color: C.fg }}>{p.name}</span>
                      </div>
                    </td>
                    <td style={{ padding: '11px 16px', fontSize: 13, color: C.mutedFg }}>{p.sku}</td>
                    <td style={{ padding: '11px 16px' }}>
                      <span style={{ fontSize: 12, background: C.categoryBg, color: C.categoryText, fontWeight: 500, borderRadius: 6, padding: '2px 8px' }}>{p.category}</span>
                    </td>
                    <td style={{ padding: '11px 16px', fontSize: 14, textAlign: 'right', fontVariantNumeric: 'tabular-nums', color: C.fg }}>{p.onHand}</td>
                    <td style={{ padding: '11px 16px', fontSize: 14, textAlign: 'right', fontVariantNumeric: 'tabular-nums', color: C.mutedFg }}>{p.committed}</td>
                    <td style={{ padding: '11px 16px', textAlign: 'right' }}><StockBadge available={available} threshold={p.threshold} /></td>
                    <td style={{ padding: '11px 16px', fontSize: 14, textAlign: 'right', fontWeight: 500, fontVariantNumeric: 'tabular-nums', color: C.fg }}>{p.price}</td>
                  </tr>
                );
              })}
            </tbody>
...
Object.assign(window, { InventoryScreen, PRODUCTS });
