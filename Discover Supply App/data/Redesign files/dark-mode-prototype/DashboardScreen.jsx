...
function DashboardScreen() {
  ...
  return (
    <div style={{ padding: '28px 32px', maxWidth: 1100 }}>
      <PageHeader title="Dashboard" subtitle="Welcome to Acme Distributors." />
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {recentOrders.map((o, i) => (
              <tr key={o.id}
                style={{ borderBottom: i < recentOrders.length - 1 ? `1px solid ${C.border}` : 'none', transition: 'background 100ms' }}
                ...
                <td style={{ padding: '12px 16px', fontSize: 14, color: C.fg }}>
                  {o.store
                    ? <><div>{o.store}</div>{o.code && <div style={{ fontSize: 11, color: C.mutedFg }}>{o.code}</div>}</>
                    : <span style={{ color: C.mutedFg }}>—</span>}
                </td>
                <td style={{ padding: '12px 16px' }}><StageBadge name={o.stage} color={o.stageColor} /></td>
