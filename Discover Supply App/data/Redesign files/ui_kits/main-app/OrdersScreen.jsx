...
const STAGES = [
  { id: 'draft',     name: 'Draft',     color: '#94a3b8', isTerminal: false, effect: 'none' },
  { id: 'confirmed', name: 'Confirmed', color: '#3b82f6', isTerminal: false, effect: 'commit' },
  { id: 'packed',    name: 'Packed',    color: '#f59e0b', isTerminal: false, effect: 'none' },
  { id: 'delivered', name: 'Delivered', color: '#10b981', isTerminal: false, effect: 'consume' },
  { id: 'paid',      name: 'Paid',      color: '#8b5cf6', isTerminal: true,  effect: 'mark_paid' },
  { id: 'cancelled', name: 'Cancelled', color: '#ef4444', isTerminal: true,  effect: 'release' },
];
  ...
  return (
    <div style={{ padding: '28px 32px', maxWidth: 1100 }}>
      <PageHeader
        ...
        actions={
          <>
            <Btn variant="outline" size="sm">Print</Btn>
            <Btn variant="outline" size="sm">Create invoice</Btn>
          </>
        }
      />
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', gap: 20, marginTop: 24 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <Card style={{ overflow: 'hidden' }}>
            <div style={{ padding: '14px 20px', borderBottom: `1px solid ${C.border}`, fontSize: 15, fontWeight: 600 }}>Items</div>
              ...
              <tbody>
                {order.lines.map((l, i) => (
                  <tr key={i} style={{ borderBottom: i < order.lines.length - 1 ? `1px solid ${C.border}` : 'none' }}>
                    <td style={{ padding: '11px 16px', fontSize: 14, fontWeight: 500 }}>
                    ...
                    <td style={{ padding: '11px 16px', fontSize: 14, textAlign: 'right', fontVariantNumeric: 'tabular-nums' }}>{l.qty}</td>
                    <td style={{ padding: '11px 16px', fontSize: 14, textAlign: 'right', fontVariantNumeric: 'tabular-nums' }}>{l.price}</td>
                    <td style={{ padding: '11px 16px', textAlign: 'right' }}><StockBadge available={l.available} /></td>
                    <td style={{ padding: '11px 16px', fontSize: 14, fontWeight: 600, textAlign: 'right', fontVariantNumeric: 'tabular-nums' }}>{l.lineTotal}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
function OrdersScreen({ onSelectOrder, selectedOrderId, onBack, onStageChange }) {
  ...
  return (
    <div style={{ padding: '28px 32px', maxWidth: 1100 }}>
      <PageHeader title="Orders" subtitle={`${filtered.length} order${filtered.length === 1 ? '' : 's'}`}
        actions={<Btn onClick={() => {}}><Icons.Plus /> New order</Btn>} />

      <div style={{ display: 'flex', gap: 8, margin: '20px 0', flexWrap: 'wrap' }}>
        <Input placeholder="Search order # or store…" value={q} onChange={e => setQ(e.target.value)}
      ...
      {filtered.length === 0 ? (
        <Card>
          <EmptyState icon={Icons.Cart} title="No orders match your filters"
            description="Clear or adjust the search and stage filters to widen the list." />
        </Card>
      ) : (
        <Card style={{ overflow: 'hidden' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {filtered.map((o, i) => {
                ...
                return (
                  <tr key={o.id} style={{ borderBottom: i < filtered.length - 1 ? `1px solid ${C.border}` : 'none', cursor: 'pointer', transition: 'background 100ms' }}
