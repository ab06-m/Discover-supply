...
const Icon = ({ d, children, size = 16, stroke = "currentColor", fill = "none", strokeWidth = 2, viewBox = "0 0 24 24", style: s }) => (
  <svg width={size} height={size} viewBox={viewBox} fill={fill} stroke={stroke} strokeWidth={strokeWidth} strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0, ...s }}>
    {d ? <path d={d} /> : children}
  </svg>
);
...
const Icons = {
  ...
  Package: () => <Icon><path d="M20 7H4a2 2 0 0 0-2 2v10a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V9a2 2 0 0 0-2-2z"/><path d="M16 21V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v16"/></Icon>,
  ...
  Settings: () => <Icon><circle cx="12" cy="12" r="3"/><path d="M19.07 4.93a10 10 0 0 1 0 14.14M4.93 4.93a10 10 0 0 0 0 14.14"/></Icon>,
...
function Btn({ children, variant = 'default', size = 'default', onClick, disabled, style: s, type = 'button' }) {
  ...
  const sizes = { default: { height: 40, padding: '0 16px', fontSize: 14 }, sm: { height: 36, padding: '0 12px', fontSize: 13 }, lg: { height: 44, padding: '0 24px', fontSize: 15 }, icon: { height: 40, width: 40, padding: 0, justifyContent: 'center' } };
  const variants = {
...
function Card({ children, style: s, onClick, hoverable }) {
  ...
  return (
    <div onClick={onClick} onMouseEnter={() => hoverable && setHov(true)} onMouseLeave={() => hoverable && setHov(false)}
      style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 10, boxShadow: hov ? cardHoverShadow : cardShadow, cursor: onClick ? 'pointer' : 'default', ...s }}>
      {children}
    </div>
  );
}
...
function StockBadge({ available, threshold = 5, unit = 'Pcs' }) {
  if (available <= 0) return <span style={{ fontSize: 12, fontWeight: 600, padding: '1px 6px', borderRadius: 4, background: C.destructiveBg, color: 'hsl(0 84% 40%)' }}>Out: 0 {unit}</span>;
  if (available <= threshold) return <span style={{ fontSize: 12, fontWeight: 600, padding: '1px 6px', borderRadius: 4, background: C.warningBg, color: 'hsl(38 92% 32%)' }}>Low: {available} {unit}</span>;
  return <span style={{ fontSize: 12, fontWeight: 600, padding: '1px 6px', borderRadius: 4, background: C.successBg, color: 'hsl(142 71% 28%)' }}>{available} {unit}</span>;
}
  ...
  return (
    <div style={{ position: 'relative', display: 'inline-flex', alignItems: 'center' }}>
      ...
      <input type={type} placeholder={placeholder} value={value} onChange={onChange}
        style={{ fontFamily: 'inherit', fontSize: 14, color: C.fg, background: C.card, border: `1px solid ${C.border}`, borderRadius: 8, height: 40, padding: icon ? '0 12px 0 34px' : '0 12px', width: '100%', outline: 'none', ...s }}
        onFocus={e => { e.target.style.borderColor = C.primary; e.target.style.boxShadow = '0 0 0 3px hsl(221 83% 53% / 0.12)'; }}
...
function Select({ value, onChange, children, style: s }) {
  return (
    <select value={value} onChange={onChange}
      style={{ fontFamily: 'inherit', fontSize: 14, color: C.fg, background: C.card, border: `1px solid ${C.border}`, borderRadius: 8, height: 40, padding: '0 12px', outline: 'none', cursor: 'pointer', width: '100%', ...s }}>
      {children}
    </select>
  );
}
  ...
  return (
    <Card style={{ padding: 20 }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 8 }}>
        ...
        {IconComp && (
          <div style={{ width: 32, height: 32, borderRadius: 8, background: (iconColor || C.primary) + '18', display: 'flex', alignItems: 'center', justifyContent: 'center', color: iconColor || C.primary }}>
            <IconComp />
          </div>
        )}
      </div>
      ...
      {(trend || hint) && (
        <div style={{ marginTop: 6, display: 'flex', alignItems: 'center', gap: 6 }}>
          {trend && <span style={{ background: trend.positive ? C.successBg : C.destructiveBg, color: trend.positive ? 'hsl(142 71% 28%)' : 'hsl(0 84% 40%)', fontSize: 11, fontWeight: 600, borderRadius: 4, padding: '1px 6px' }}>{trend.value}</span>}
...
Object.assign(window, { Icons, C, cardShadow, cardHoverShadow, Btn, Card, Badge, StageBadge, StockBadge, Input, Select, PageHeader, EmptyState, StatCard, TabNav });
